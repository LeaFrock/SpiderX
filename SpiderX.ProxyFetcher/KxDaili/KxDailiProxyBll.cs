...
namespace SpiderX.ProxyFetcher
{
	public class KxDailiProxyBll : ProxyBll
	{
		...
		public override void Run()
		{
			...
			{
				var entities = GetProxyEntities(webClient, KxDailiProxyApiProvider.IpUrlTemplate, 10);
				if (entities.Count < 1)
				{
					return;
				}
		...
		{
			List<SpiderProxyEntity> entities = new List<SpiderProxyEntity>(maxPage * 10);
		...
		private static string GetRequestUrl(string urlTemplate, int page) => string.Format(urlTemplate, page.ToString());
	}
}